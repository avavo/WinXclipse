...
- **FSR 1.1 gave no FPS gain on GPU-bound games (RE2)**: `EASU` was compositing a bilinear-downscaled image instead of reducing the guest's rendered pixels. The container now boots the X desktop at `display/factor` (e.g. `1280x720` `Performance 2.0x` → `640x360` guest, `Ultra 2.5x` → `512x288`) so the game shades `2.2x-6.25x` fewer pixels and `EASU` performs the single upscale to the panel. Live preset changes in `XServerDisplayActivity.applyFsrRuntime` now actually resize the X desktop via `XServer.updateScreenSize`/`WindowManager.resizeRootWindow` (desktop + `isApplicationWindow` game window) instead of only swapping shaders, fixing the reported `90-100% GPU` with `0 FPS` delta. Defaults: selecting `Texture Filter = FSR` now implies `FSR Upscale = On` (`XServerDisplayActivity.resolveFsrState`, `VideoConfigDialog`, `ContainerDetailFragment`, `ShortcutSettingsDialog`) so new configs get performance out of the box; `sharpen-only` remains available by explicitly storing `fsrUpscale=0`.
