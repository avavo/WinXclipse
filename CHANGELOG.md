...
- **Community configs could disappear or fail with "Invalid or unsupported community config"**: the catalog no longer depends exclusively on the rate-limited GitHub API and falls back to the public release assets page, a saved catalog index, and validated cached ZIPs. Selected configs are staged in persistent app files instead of the shared cache directory, so navigating between screens cannot delete the ZIP before import. Exact bundled-content identities are recognized correctly, including FEXCore 2608 and Box64 0.4.3.
- **Imported community containers sometimes opened and immediately closed**: transient import state is cleared after installation and content resolution is defensive against damaged packages or profiles. Container creation and launch now wait for required content operations to finish without reusing stale import state.
- **Wine-created shortcuts were missing or unusable**: Windows `.lnk` parsing now accepts Wine variants and resolves relative, quoted, environment-based, and Unix/Wine paths more reliably. Shortcut discovery, executable targeting, naming, and artwork refresh were aligned with the working Winlator implementations.
- **Touchscreen mouse could not reliably use Wine context menus or create shortcuts**: touchpad button handling was adapted from Winlator Mali, including pointer grabs, left/right press-release ordering, drag state, and X11 `GrabPointer` event-mask parsing. Right-click no longer stalls selection in Wine menus.
- Deleting a container no longer performs its filesystem cleanup on the UI thread, removing the visible interface hitch.
- The startup controller-assignment notice was removed completely.
- Bundled Box64 was replaced with the requested `0.4.3-260519` package and the default content identity now matches it.
...
- Community configs now group equivalent game names case-insensitively, trim trailing whitespace, and normalize Arabic/Roman sequel numbers through X. Every ZIP in a group is checked for an embedded cover before browser search is used. Users can replace the displayed community cover manually.
- The community-config flow is English and AMOLED-themed. Game cards install by game name; multiple configs first show device-model choices, then expand into phone, RAM, GPU, CPU, expected FPS, tester, notes, and an optional on-demand container-configuration view.
- Community export removes the version suffix from the filename, requires a game/container name, preserves the detected phone model, and records the Discord handle, expected FPS, hardware metadata, notes, content requirements, and an optional embedded cover.
- Input Controls now exposes the same `Auto-grab controller (plug & play)`, master `Vibration`, and vibration-test controls as Controller Manager. The obsolete touchscreen-haptic option and its runtime feedback path were removed.
