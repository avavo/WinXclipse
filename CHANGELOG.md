...
- Winlator HUD can show the container graphics API and the phone's physical GPU; Apex telemetry separates source Presents received from the game from real/generated draws completed by the compositor, instead of deriving both values from Android display ticks or a theoretical multiplier. Optional rows expose Frame Generation latency, backend/fallback state and failures.
...
- Normal and Essential Wine startup disable PlugPlay, RpcSs and tablet input. Aggressive keeps controller-critical PlugPlay available but also disables RpcSs and tablet input, removing the idle `rpcss.exe`/`tabtip.exe` pair and matching Ludashi's seven-to-eight-process baseline instead of starting ten processes before a game opens.
