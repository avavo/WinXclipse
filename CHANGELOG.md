...
- `Wrapper-Default` keeps the hybrid Wrapper-Default + July13 stack for normal use and for ASTC/ETC2 transcode (RE3 Remake incluso).
...
- Explicit ASTC/ETC2 requests force decode instead of allowing wrapper driver-ID auto-detection to skip it and report `ARMED`, `LOADED`, `ACTIVE`, or a specific capability/encode/resource error in the HUD. `ACTIVE` requires a logged transcode operation. The RE3 white ETC2/black ASTC regression was traced to the alternate wrapper/layer pair (`wrapper-default-transcode`/`leegao_bcn_transcode_compat`), whose wrapper exposes no `WRAPPER_BCN/ASTC` nor transcode path so the layer disables transcode as unsupported: ETC2/ASTC transcode now always uses the hybrid Wrapper-Default + July13 pair with forced decode (`BCN_COMPUTE_AUTO=0`, storage-image + profiling), and containers stuck on the broken pair refresh automatically.
