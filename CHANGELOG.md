...
- Fixed multipliers use 0.5x steps from 1.5x through 5x; Automatic mode uses the entered target FPS.
...
- DXVK-reported video memory is capped again: a set Graphics Driver Max Device Memory reaches `dxgi.maxDeviceMemory/SharedMemory` as in 0.7.6, and unset values fall back to a RAM-tier default (2048/2048 MB up to 6 GB RAM, 3072/2048 MB above), fixing the GTA V city-driving OOM freeze/crash on shared-memory Xclipse GPUs. Reported memory never exceeds the hardest active cap (driver or Experimental Performance VRAM cap), because planning beyond the real budget unloads textures and crashes; hard caps alone stay off unless configured.
...
- RAGE sessions without a custom `dxvk.conf`, manual `DXVK_CONFIG` or explicit hard cap now report a 512 MB VRAM pool (`dxgi.maxDeviceMemory = 512`) instead of the RAM-tier default. Community testing confirmed the game then holds total RAM around 80% instead of the 85-90% band where Android's `lmkd` reaps it; the streaming flags from RAM Fix are required alongside, so the small pool applies only with RAM Fix enabled.
- RAM Fix is now configurable and general: the DXVK + VKD3D dialog gained a `RAM Fix pool` selector (`Auto`/512/1024/1536/2048/3072/4096 MB, per container and per shortcut) that overrides only the reported device memory; shared/d3d9 stay conservative and a VRAM hard cap still clamps the reported value. With RAM Fix on, every game without a custom conf gets a conservative pool (1024/1024 MB up to 6 GB RAM, 1536/1024 MB above) instead of the legacy 2048/3072 MB tier, and the RAGE small pool is tightened to 512/1024/2048 MB after full-phone OOMs in testing. A `dxvk.conf` next to the game's `.exe` governs its own memory keys like in Ludashi/vanilla (per-key merge: safety keys always apply, file memory keys survive), and imported `dxvk.conf` files with a 512 MB cap are no longer flagged — only sub-512 MB caps are sanitized, up to 1024 MB instead of 2048 MB.
