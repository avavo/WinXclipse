...
final class LSFGComputeMaterial {
    ...
    private int compileComputeShader() {
        String source = "#version 310 es\n"
                ...
                + "uniform int quality;\n"
                // RGBA8 image load/store is mandatory in GLES 3.1 and avoids
                // vendor-dependent half-float image/FBO failures. Motion is
                // encoded around 0.5 with a +/-0.125 normalized range.
                + "layout(rgba8, binding=0) uniform writeonly highp image2D motionVectorOutput;\n"
                ...
                + " vec3 c=textureLod(currFrame,uv,0.0).rgb;\n"
                + " float d=distance(c,textureLod(prevFrame,uv,0.0).rgb)*0.57735;\n"
                + " if(d<0.008){imageStore(motionVectorOutput,p,vec4(0.5,0.5,1.0,1.0));return;}\n"
                + " float cut=d>0.70?1.0:0.0, best=d; vec2 mv=vec2(0.0);\n"
                ...
                + "  for(int j=0;j<4;j++){vec2 candidate=mv+o[j]*ts;float sad=distance(c,textureLod(prevFrame,clamp(uv+candidate,0.0,1.0),0.0).rgb)*0.57735;if(sad<best){best=sad;mv=candidate;}}}}\n"
                + " vec2 old=(textureLod(mvHistoryTexture,uv,0.0).rg-0.5)*0.25;\n"
                + " vec2 hist=(textureLod(mvHistoryTexture,clamp(uv-old,0.0,1.0),0.0).rg-0.5)*0.25;\n"
                + " float conf=(1.0-cut)*(1.0-clamp(best*2.5,0.0,1.0));\n"
                + " vec2 stable=mix(hist,mv,0.8)*smoothstep(0.10,0.55,conf);\n"
                + " vec2 encoded=clamp(stable*4.0+0.5,0.0,1.0);\n"
                + " imageStore(motionVectorOutput,p,vec4(encoded,conf,1.0));}\n";
