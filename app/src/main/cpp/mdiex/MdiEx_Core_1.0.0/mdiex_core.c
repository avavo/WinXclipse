...
static MdiExDeviceInfo g_info = {
    "unknown", "unknown", "unknown", "unknown", MDIEX_CPU_UNKNOWN, 0, "ram_unknown", "native_fallback"
};

typedef enum MdiExInitSource {
    MDIEX_INIT_NONE = 0,
    MDIEX_INIT_AUTO = 1,
    MDIEX_INIT_META = 2,
    MDIEX_INIT_META_FALLBACK = 3
} MdiExInitSource;

static atomic_int g_initialized = 0;
static atomic_int g_init_source = MDIEX_INIT_NONE;
static atomic_uint g_hints[MDIEX_HINT_MAX];
static pthread_mutex_t g_info_lock = PTHREAD_MUTEX_INITIALIZER;
static _Thread_local MdiExDeviceInfo g_tls_info;

static void core_log(const char* msg) {
#ifdef __ANDROID__
    __android_log_print(ANDROID_LOG_INFO, "MdiExCore", "%s", msg ? msg : "");
#else
    fprintf(stderr, "[MdiExCore] %s\n", msg ? msg : "");
#endif
}

void mdiex_log_line(const char* tag, const char* message) {
#ifdef __ANDROID__
    __android_log_print(ANDROID_LOG_INFO, tag ? tag : "MdiEx", "%s", message ? message : "");
#else
    fprintf(stderr, "[%s] %s\n", tag ? tag : "MdiEx", message ? message : "");
#endif
}
...
static void reset_info(void) {
    ...
    g_info.cpu_class = MDIEX_CPU_UNKNOWN;
    g_info.ram_gb = 0;
...
static void classify_from_soc(const char* soc_raw) {
    ...
    if (contains_any(s, "1480", "s5e8845", "exynos1480")) {
        ...
        g_info.cpu_class = MDIEX_CPU_OCTA;
        ...
        g_info.cpu_class = MDIEX_CPU_DECA;
...
static void publish_profile_env_unlocked(void) {
    ...
    /* Do not overwrite a profile explicitly set by the user/emulator. If MdiEx
...
static int mdiex_init_common(const char* meta_path, MdiExInitSource source) {
    ...
    if (atomic_load(&g_initialized)) {
        const int can_meta_override =
            (source == MDIEX_INIT_META) &&
            (old_source == MDIEX_INIT_AUTO || old_source == MDIEX_INIT_META_FALLBACK);
        if (!can_meta_override) {
            pthread_mutex_unlock(&g_info_lock);
            return 1;
        }
        mdiex_log_line("MdiExCore", "meta init overriding earlier auto/meta-fallback init");
    }
    ...
    if (meta_path && meta_path[0]) {
        ...
        if (meta) {
            ...
            free(meta);
            if (!used_meta) {
                mdiex_log_line("MdiExCore", "meta parsed, but no explicit known Exynos target found; using auto detection");
            }
        } else {
            mdiex_log_line("MdiExCore", "meta path could not be read; using auto detection");
        }
    }
    ...
    atomic_store(&g_init_source, used_meta ? MDIEX_INIT_META :
                 ((source == MDIEX_INIT_META && meta_read_ok) ? MDIEX_INIT_META_FALLBACK : MDIEX_INIT_AUTO));
    ...
    snprintf(msg, sizeof(msg), "soc=%s gpu=%s arch=%s cpu=%s ram=%dGB ram_class=%s profile=%s source=%s",
             ...
             used_meta ? "meta" : ((source == MDIEX_INIT_META && meta_read_ok) ? "meta-fallback" : "auto"));
...
int mdiex_init_from_meta(const char* meta_path) {
    if (!meta_path || !meta_path[0]) {
        mdiex_log_line("MdiExCore", "empty meta path; using auto detection");
        return mdiex_init_auto();
    }
    return mdiex_init_common(meta_path, MDIEX_INIT_META);
}

int mdiex_init_auto(void) {
    return mdiex_init_common(NULL, MDIEX_INIT_AUTO);
}

int mdiex_init_from_env_or_auto(void) {
    const char* meta = getenv("MDIEX_META_PATH");
    if (meta && meta[0]) return mdiex_init_from_meta(meta);
    return mdiex_init_auto();
}
...
const char* mdiex_get_soc_name(void) { return mdiex_get_device_info()->soc; }
const char* mdiex_get_gpu_name(void) { return mdiex_get_device_info()->gpu; }
const char* mdiex_get_gpu_arch(void) { return mdiex_get_device_info()->gpu_arch; }
const char* mdiex_get_cpu_topology(void) { return mdiex_get_device_info()->cpu_topology; }
int mdiex_get_cpu_class(void) { return mdiex_get_device_info()->cpu_class; }
int mdiex_get_ram_gb(void) { return mdiex_get_device_info()->ram_gb; }
const char* mdiex_get_ram_class(void) { return mdiex_get_device_info()->ram_class; }
const char* mdiex_get_profile_name(void) { return mdiex_get_device_info()->package_profile; }

void mdiex_send_hint(int hint) {
    if (hint <= 0 || hint >= MDIEX_HINT_MAX) return;
...
int mdiex_get_hint_state(int hint) {
    if (hint > 0 && hint < MDIEX_HINT_MAX) return atomic_load(&g_hints[hint]) > 0u;
    return 0;
}

int mdiex_consume_hint(int hint) {
    if (hint > 0 && hint < MDIEX_HINT_MAX) return (int)atomic_exchange(&g_hints[hint], 0u);
    return 0;
}

void mdiex_clear_hint(int hint) {
    if (hint > 0 && hint < MDIEX_HINT_MAX) atomic_store(&g_hints[hint], 0u);
}

__attribute__((constructor)) static void mdiex_core_ctor(void) {
    const char* meta = getenv("MDIEX_META_PATH");
    if (meta && meta[0]) {
        mdiex_init_from_meta(meta);
    } else {
        mdiex_log_line("MdiExCore", "constructor ready; deferred auto init until first query");
    }
}
