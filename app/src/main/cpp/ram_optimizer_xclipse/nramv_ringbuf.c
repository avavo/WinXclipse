...
static nramv_ringbuf_t *g_buf    = NULL;
...
static inline int nramv_memfd_create(const char *name, unsigned int flags)
{
...
int nramv_ringbuf_init(void)
{
    ...
    fd = nramv_memfd_create("nramv_events", MFD_CLOEXEC);
    if (fd < 0) {
        pthread_mutex_unlock(&g_ringbuf_lock);
        return NRAMV_ERR_NODEV;
    }

    if (ftruncate(fd, (off_t)NRAMV_RINGBUF_SIZE) != 0) {
        close(fd);
        pthread_mutex_unlock(&g_ringbuf_lock);
        return NRAMV_ERR_NODEV;
    }
    ...
    memset(map, 0, NRAMV_RINGBUF_SIZE);

    g_buf = (nramv_ringbuf_t *)map;
    g_buf_fd = fd;

    g_buf->magic      = NRAMV_RINGBUF_MAGIC;
    g_buf->version    = NRAMV_RINGBUF_VERSION;
    g_buf->slot_count = NRAMV_RINGBUF_SLOTS;
    g_buf->slot_size  = sizeof(nramv_event_t);
...
void nramv_ringbuf_push(nramv_event_kind_t kind,
                        ...
                        int gtt_pressure_pct)
{
    ...
    slot_idx = events % NRAMV_RINGBUF_SLOTS;
...
int nramv_ringbuf_read(nramv_event_t *out, int count)
{
    ...
    available = (events < NRAMV_RINGBUF_SLOTS)
              ? events
              : NRAMV_RINGBUF_SLOTS;

    n = (count < (int)available) ? count : (int)available;
    start = (events + NRAMV_RINGBUF_SLOTS - (uint32_t)n) %
            NRAMV_RINGBUF_SLOTS;

    for (int i = 0; i < n; i++) {
        uint32_t idx = (start + (uint32_t)i) % NRAMV_RINGBUF_SLOTS;
...
void nramv_ringbuf_destroy(void)
{
    ...
    if (g_buf) {
        munmap(g_buf, NRAMV_RINGBUF_SIZE);
        g_buf = NULL;
    }
