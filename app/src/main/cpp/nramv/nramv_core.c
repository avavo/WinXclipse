...
#include "nramv_feedback.h"
#include "nramv_thermal.h"

static atomic_int g_initialized = 0;

static pthread_mutex_t g_state_lock = PTHREAD_MUTEX_INITIALIZER;

static nramv_profile_t g_profile = NRAMV_PROFILE_MEDIUM;

/* cooldown monotônico */
#define NRAMV_FLUSH_COOLDOWN_SEC 2

#define NRAMV_MAJFLT_SAMPLE_MS        80
#define NRAMV_MAJFLT_RATE_THRESHOLD   5
#define NRAMV_MAJFLT_SUSPEND_SEC      2

/* hysteresis térmica */
#define NRAMV_THERMAL_HYSTERESIS_SEC 5

/* moderação quando o processo já está usando swap */
#define NRAMV_VMSWAP_AGGRESSIVE_LIMIT_KB (64 * 1024)
...
static int flush_cooldown_ok(void)
{
    ...
    return (mono_sec() - last) >= NRAMV_FLUSH_COOLDOWN_SEC;
}
 ...
 * nramv_flush() e nramv_diagnose(). O resto são guardas de segurança:
 * thermal, majflt e VmSwap.
 * ───────────────────────────────────────────── */

static unsigned int nramv_compute_avail_threshold(const nramv_psi_t *psi,
                                                  int feedback_adj)
{
...
static unsigned long nramv_effective_denom_kb(unsigned long total_kb,
                                              unsigned long available_kb)
{
    ...
    cg_limit = nramv_cgroup_limit_kb();
...
static nramv_profile_t nramv_choose_base_profile(const nramv_stats_t *ram,
                                                 int frag,
                                                 const nramv_psi_t *psi,
                                                 nramv_gtt_level_t gtt_level)
{
    ...
    feedback_adj = nramv_feedback_get_threshold_adjustment();

    avail_threshold =
        nramv_compute_avail_threshold(psi, feedback_adj);

    denom_kb =
        nramv_effective_denom_kb(ram->total_kb, ram->available_kb);
    ...
    if (frag > 35) {
        return
            (gtt_level >= NRAMV_GTT_AGGRESSIVE)
            ? NRAMV_PROFILE_AGGRESSIVE
            : NRAMV_PROFILE_MEDIUM;
    }

    if (gtt_level >= NRAMV_GTT_AGGRESSIVE)
        return NRAMV_PROFILE_AGGRESSIVE;

    if (gtt_level >= NRAMV_GTT_MEDIUM)
        return NRAMV_PROFILE_MEDIUM;

    return NRAMV_PROFILE_LIGHT;
}

static nramv_profile_t nramv_apply_thermal_guard(nramv_profile_t profile)
{
    long long now;
    nramv_thermal_level_t thermal;

    now = mono_sec();
    thermal = nramv_thermal_get_level();

    if (thermal >= NRAMV_THERMAL_HOT) {

        atomic_store(&g_last_thermal_hot, now);

        if (profile == NRAMV_PROFILE_AGGRESSIVE)
            return NRAMV_PROFILE_MEDIUM;

        if (profile == NRAMV_PROFILE_MEDIUM)
            return NRAMV_PROFILE_LIGHT;
...
static nramv_profile_t nramv_apply_thermal_guard_passive(nramv_profile_t profile,
                                                           int thermal_level)
{
    long long now;
    long long hot_last;

    if (thermal_level >= NRAMV_THERMAL_HOT) {
        if (profile == NRAMV_PROFILE_AGGRESSIVE)
            return NRAMV_PROFILE_MEDIUM;
    ...
    if (hot_last != 0 &&
        (now - hot_last) < NRAMV_THERMAL_HYSTERESIS_SEC) {
...
static nramv_profile_t nramv_apply_swap_guard(nramv_profile_t profile)
{
    long long vmswap;
    ...
    vmswap = nramv_self_vmswap_kb();

    if (vmswap > NRAMV_VMSWAP_AGGRESSIVE_LIMIT_KB)
        return NRAMV_PROFILE_MEDIUM;
...
int nramv_init(void)
{
    ...
    if (nramv_read_meminfo(&stats) != NRAMV_OK) {
        pthread_mutex_unlock(&g_state_lock);
        return NRAMV_ERR_INIT;
    }

    g_profile = NRAMV_PROFILE_MEDIUM;

    nramv_feedback_init();
    nramv_thermal_init();

    {
        int r = nramv_mallopt_apply(NRAMV_PROFILE_MEDIUM);

        if (r != NRAMV_OK && r != NRAMV_ERR_NODEV) {
            pthread_mutex_unlock(&g_state_lock);
            return r;
        }
    }
...
int nramv_apply_profile(nramv_profile_t profile)
{
    int ret;

    if ((int)profile < 0 || profile > NRAMV_PROFILE_MAX_VALID)
    ...
    ret = nramv_mallopt_apply(profile);

    if (ret != NRAMV_OK && ret != NRAMV_ERR_NODEV) {
        pthread_mutex_unlock(&g_state_lock);
        return ret;
    }
...
int nramv_flush(void)
{
    nramv_stats_t     ram;
    nramv_gtt_state_t gtt;
    nramv_psi_t       psi;

    nramv_profile_t   flush_profile;
    nramv_gtt_level_t gtt_level;
 ...
 */
#define NRAMV_FLUSH_RETURN_UNLOCK(value) \
    do { \
    ...
    /* recheck após lock */
    if (!atomic_load(&g_initialized)) {
        NRAMV_FLUSH_RETURN_UNLOCK(NRAMV_ERR_INIT);
    }
    ...
    if (!flush_cooldown_ok()) {
        ...
        if (nramv_read_meminfo(&quick) == NRAMV_OK &&
            quick.total_kb > 0 &&
        ...
        if (!emergency) {
            NRAMV_FLUSH_RETURN_UNLOCK(NRAMV_OK);
        }
    }

    ret = nramv_read_meminfo(&ram);

    if (ret != NRAMV_OK) {
        NRAMV_FLUSH_RETURN_UNLOCK(ret);
    }

    frag = nramv_read_fragmentation();

    memset(&gtt, 0, sizeof(gtt));
    (void)nramv_gtt_read(&gtt);

    gtt_level = nramv_gtt_classify(&gtt);

    memset(&psi, 0, sizeof(psi));
    (void)nramv_psi_read(&psi);

    flush_profile =
        nramv_choose_base_profile(&ram, frag, &psi, gtt_level);

    flush_profile =
        nramv_apply_thermal_guard(flush_profile);
     ...
     */
    if (flush_profile >= NRAMV_PROFILE_MEDIUM) {
        ...
        if (last_suspend != 0 &&
            ...
            NRAMV_MAJFLT_SUSPEND_SEC) {

            flush_profile = NRAMV_PROFILE_LIGHT;
        }
            ...
            if (flt_before >= 0) {
                ...
                if (flt_after >= flt_before) {
                    ...
                    rate =
                        (long)
                        (flt_delta * 1000L /
                        NRAMV_MAJFLT_SAMPLE_MS);

                    if (rate >=
                        NRAMV_MAJFLT_RATE_THRESHOLD) {

                        nramv_ringbuf_push(
                            NRAMV_EVT_MAJFLT_SKIP,
                            flush_profile,
                            gtt_level,
                            -1,
     ...
     */
    flush_profile = nramv_apply_swap_guard(flush_profile);

    /* snapshot de RSS antes do flush */
    nramv_feedback_snapshot_t fb_snap;
    nramv_feedback_pre_flush(&fb_snap);

    {
        int mr = nramv_mallopt_apply(flush_profile);

        if (mr != NRAMV_OK &&
            mr != NRAMV_ERR_NODEV) {

            NRAMV_FLUSH_RETURN_UNLOCK(mr);
        }
    }

    ret = nramv_heap_compact(flush_profile);
    ...
    nramv_feedback_post_flush(&fb_snap, flush_profile);
     ...
     */
    {
        ...
        long long vsz = nramv_self_vsz_kb();

        if (rss > 0 && vsz > 0) {
            if ((rss * 100 / vsz) < 30 ||
                nramv_feedback_get_ineffective_count() >= 2) {

                /*
                 * NRAMV_ERR_NODEV esperado em bionic.
                 * Descarte explícito.
                 */
                (void)nramv_mallopt_apply_perturb();
            }
        }
    }

    ret = nramv_gtt_react_level(gtt_level);
    ...
    {
        nramv_stats_t ram_after;
        unsigned long after_kb = 0;

        if (nramv_read_meminfo(&ram_after) == NRAMV_OK)
...
#undef NRAMV_FLUSH_RETURN_UNLOCK
...
int nramv_diagnose(nramv_diagnostic_t *out)
{
    nramv_psi_t psi;
    nramv_gtt_state_t gtt;
    nramv_gtt_level_t gtt_level;
    nramv_profile_t suggested;
    ...
    out->ram.fragmentation = nramv_read_fragmentation();
    ...
    out->psi_level = nramv_psi_level(&psi);
    ...
    out->feedback_threshold_adjustment =
        nramv_feedback_get_threshold_adjustment();

    /* Diagnose é passivo: não tenta reinicializar thermal aqui. */
    out->thermal_temp_c = nramv_thermal_get_temp();
    out->thermal_level = (int)nramv_thermal_get_level();

    suggested =
        nramv_choose_base_profile(&out->ram,
     ...
     */
    suggested =
        nramv_apply_thermal_guard_passive(suggested,
                                          out->thermal_level);

    /* Espelha a moderação por VmSwap do flush. */
    suggested = nramv_apply_swap_guard(suggested);

    out->suggested_profile = suggested;

    out->would_act =
        (out->psi_level > 0 ||
         gtt_level != NRAMV_GTT_NORMAL ||
         out->ram.fragmentation > 35 ||
         suggested != NRAMV_PROFILE_LIGHT)
        ? 1
        : 0;
...
int nramv_get_stats(nramv_stats_t *out)
{
    int ret;
    ...
    ret = nramv_read_meminfo(out);

    if (ret != NRAMV_OK)
        return ret;

    out->fragmentation = nramv_read_fragmentation();
...
#define NRAMV_CORE_STRINGIFY_IMPL(x) #x
#define NRAMV_CORE_STRINGIFY(x)      NRAMV_CORE_STRINGIFY_IMPL(x)

const char *nramv_version(void)
{
    static const char version[] =
        NRAMV_CORE_STRINGIFY(NRAMV_VERSION_MAJOR) "."
        NRAMV_CORE_STRINGIFY(NRAMV_VERSION_MINOR) "."
        NRAMV_CORE_STRINGIFY(NRAMV_VERSION_PATCH);
