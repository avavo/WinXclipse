...
unsigned long long wxp_surface_get_frame_count(void) { return atomic_load(&g_frames); }
unsigned long long wxp_surface_get_avg_frame_delta_ns(void) { return atomic_load(&g_avg_delta_ns); }
unsigned long long wxp_surface_get_jitter_ns(void) { return atomic_load(&g_jitter_ns); }

int wxp_surface_get_frame_pacing_state(void) {
    unsigned long long avg = atomic_load(&g_avg_delta_ns);
    unsigned long long jitter = atomic_load(&g_jitter_ns);
    if (avg == 0) return WXP_SURFACE_PACING_UNKNOWN;
    if (jitter > avg / 2ull) return WXP_SURFACE_PACING_BAD;
    if (jitter > avg / 4ull) return WXP_SURFACE_PACING_UNSTABLE;
    return WXP_SURFACE_PACING_STABLE;
}
...
int wxp_surface_init(void) {
    ...
    wxp_log_line("WinXclipsePolicySurface", "surface monitor initialized with frame pacing state");
    return 1;
}
__attribute__((constructor)) static void wxp_surface_ctor(void) { wxp_surface_init(); }
