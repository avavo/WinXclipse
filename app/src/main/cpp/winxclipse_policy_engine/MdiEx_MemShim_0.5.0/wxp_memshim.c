...
#ifndef WXP_MEMSHIM_ENABLE_INTERPOSE_SYMBOLS
#define WXP_MEMSHIM_ENABLE_INTERPOSE_SYMBOLS 1
#endif
...
static void wxp_memshim_poll_fragmentation_hints(void) {
    int pressure = 0;
    pressure += wxp_consume_hint(WXP_HINT_GPU_MEMORY_FRAGMENTATION);
    pressure += wxp_consume_hint(WXP_HINT_GPU_SMALL_ALLOC_CHURN);
    pressure += wxp_consume_hint(WXP_HINT_GPU_HEAP_PRESSURE);
    if (pressure > 0) {
        atomic_store(&g_fragmentation_pressure_ticks, 32);
        wxp_send_hint(WXP_HINT_MEMORY_PRESSURE);
        return;
    }
...
static size_t wxp_memshim_large_alloc_threshold(void) {
    size_t threshold = (size_t)4u * 1024u * 1024u;
    wxp_memshim_poll_fragmentation_hints();
    const int pressure_ticks = atomic_load(&g_fragmentation_pressure_ticks);
    WinXclipsePolicyXclipsePolicy policy;
    if (!wxp_get_xclipse_policy(&policy)) {
        return pressure_ticks > 0 ? (size_t)2u * 1024u * 1024u : threshold;
    }

    if (policy.ram_gb > 0 && policy.ram_gb < 8) {
        threshold = (size_t)2u * 1024u * 1024u;
    } else if (policy.thermal == WXP_THERMAL_TIGHT ||
               policy.bandwidth == WXP_BANDWIDTH_LOW ||
               policy.bandwidth == WXP_BANDWIDTH_CRITICAL ||
               policy.avoid_aggressive_upload) {
        threshold = (size_t)3u * 1024u * 1024u;
    } else if (policy.ram_gb >= 12 && policy.bandwidth == WXP_BANDWIDTH_HIGH &&
               policy.thermal == WXP_THERMAL_RELAXED) {
...
#if WXP_MEMSHIM_ENABLE_INTERPOSE_SYMBOLS
...
#define WXP_EMERGENCY_MAGIC 0x4d454d4552473336ull

typedef struct WinXclipsePolicyEmergencyHeader {
    uint64_t magic;
    size_t total_size;
    size_t user_size;
    struct WinXclipsePolicyEmergencyHeader* next;
} WinXclipsePolicyEmergencyHeader;

static pthread_mutex_t g_emergency_lock = PTHREAD_MUTEX_INITIALIZER;
static WinXclipsePolicyEmergencyHeader* g_emergency_head = NULL;
...
static void* emergency_alloc(size_t size) {
    ...
    size_t need = sizeof(WinXclipsePolicyEmergencyHeader) + size;
    ...
    WinXclipsePolicyEmergencyHeader* h = (WinXclipsePolicyEmergencyHeader*)mem;
    h->magic = WXP_EMERGENCY_MAGIC;
    h->total_size = total;
    h->user_size = size;
...
static WinXclipsePolicyEmergencyHeader* emergency_find_locked(void* ptr, WinXclipsePolicyEmergencyHeader*** prev_next_out) {
    WinXclipsePolicyEmergencyHeader** pp = &g_emergency_head;
    while (*pp) {
        WinXclipsePolicyEmergencyHeader* h = *pp;
        if ((void*)(h + 1) == ptr) {
...
static int emergency_free(void* ptr) {
    ...
    pthread_mutex_lock(&g_emergency_lock);
    WinXclipsePolicyEmergencyHeader** prev_next = NULL;
    WinXclipsePolicyEmergencyHeader* h = emergency_find_locked(ptr, &prev_next);
    if (!h || h->magic != WXP_EMERGENCY_MAGIC) {
        pthread_mutex_unlock(&g_emergency_lock);
        return 0;
    }
...
static size_t emergency_size(void* ptr) {
    if (!ptr) return 0;
    pthread_mutex_lock(&g_emergency_lock);
    WinXclipsePolicyEmergencyHeader* h = emergency_find_locked(ptr, NULL);
    size_t out = (h && h->magic == WXP_EMERGENCY_MAGIC) ? h->user_size : 0;
    pthread_mutex_unlock(&g_emergency_lock);
    return out;
}
...
#endif /* WXP_MEMSHIM_ENABLE_INTERPOSE_SYMBOLS */

void wxp_memshim_record_alloc(size_t size) {
    if (size >= wxp_memshim_large_alloc_threshold()) {
...
void* calloc(size_t nmemb, size_t size) {
    ...
    if (p && !g_in_hook && memshim_interpose_enabled()) {
        atomic_fetch_add(&g_malloc_calls, 1);
        wxp_memshim_record_alloc(total);
    }
    return p;
}
...
void* mmap(void* addr, size_t length, int prot, int flags, int fd, off_t offset) {
    ...
    if (p != MAP_FAILED && !g_in_hook && memshim_interpose_enabled()) {
        ...
        wxp_memshim_record_alloc(length);
    }
    return p;
}
...
int wxp_memshim_init(void) {
    ...
    snprintf(policy_msg, sizeof(policy_msg), "%s; large_alloc_threshold=%zu", msg,
             wxp_memshim_large_alloc_threshold());
    wxp_log_line("WinXclipsePolicyMemShim", policy_msg);
    return 1;
}

__attribute__((constructor)) static void wxp_memshim_ctor(void) { wxp_memshim_init(); }
